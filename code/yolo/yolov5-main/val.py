...
# 图像的预测结果保存为 .txt 文件，格式为归一化后的 xywh（中心点 + 宽高）坐标 + 类别（可选置信度）
...
#比较每张图像的预测框与真实框（标签），判断哪些预测是正确匹配的（即 IoU 达到指定阈值且类别匹配），返回一个布尔矩阵用于后续计算 TP/FP（真正/假正）
...
def run(
    ...
    # Initialize/load model and set device  初始化、加载模型和设备
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Plot images
        if plots and batch_i < 20:#---------------------------------------------------------------------------------保证全部val数据集呈现图片
